...
int s0=4; //Pin S0 ke Pin D3 Arduino
int s1=5; //Pin S1 ke Pin D3 Arduino
int s2=6; //Pin S2 ke Pin D3 Arduino
int s3=7; //Pin S3 ke Pin D3 Arduino
int out=8; //Pin OUT ke Pin D3 Arduino
const int LED=13;
String warna;
...
void jalan(){
  if(berhenti==1){
      analogWrite(pwm1, 0);
      analogWrite(pwm2, 0);
    }else if (arah==0 && berhenti==0) {
      analogWrite(pwm1, 80);
      analogWrite(pwm2, 0);
    }else if(arah==1 && berhenti==0) {
      analogWrite(pwm1, 0);
      analogWrite(pwm2, 80);
  }
  Serial.println(arah);
...
int rata2=(red2+green2+blue2)/3;
if(rata2>=80){
  warna="putih";
} else if(rata2>=20 && rata2 <=60){
  warna="karton";
  }else {
  warna="warna lain";
  }
...
void setup(){
   ...
   pinMode(kolam1,OUTPUT);
   pinMode(kolam2,OUTPUT);
...
void loop(){
  ...
  String currentcolor=warna;

  if(currentcolor=="putih" && berhenti==0){
    arah=1;
    berhenti=1;
    jalan();
    delay(3000);
  ...
  if(arah==0 && cek==0){
    ...
    jalan();
    while(n <5){
      digitalWrite(kolam1,HIGH);
      digitalWrite(kolam2,LOW);
      delay(1000);
      digitalWrite(kolam1,LOW);
      digitalWrite(kolam2,HIGH);
      delay(1000);
      n++;
    }
    berhenti=0;
